...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const body = generateUniv2ConfigSchema.parse(rawBody);
    ...
    const poolValidation = await validatePoolContract(body.poolAddress, body.chainId, 'uniswap-v2');
    ...
    if (!poolValidation.valid || !poolValidation.token0 || !poolValidation.token1) {
      const errorMessage = poolValidation.errors.length > 0 
        ...
        : `Pool contract validation failed. The address may not be a valid Uniswap V2 pool on chain ${body.chainId}, or it may be on a different chain.`;
      ...
      return NextResponse.json({
        ...
        suggestion: `Please verify that ${body.poolAddress} is a valid Uniswap V2 pool address on chain ${body.chainId}. If the pool is on a different chain, select the correct chain.`,
      }, { status: 400 });
    }
